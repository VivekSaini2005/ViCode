...
const saveVideoMetadata = async (req, res) => {
  try {
    // cloudinary hme bhaut jyada cheze bhejega usko nikal lo 
    const {
      problemId,
      cloudinaryPublicId,
      secureUrl,
    ...
    // Verify the upload with Cloudinary, cloudinary pr jake dekh rahe hai ki video present hai ki nhi
    const cloudinaryResource = await cloudinary.api.resource(
      cloudinaryPublicId,
      { resource_type: 'video' }
    );

    // cloudinaryResource mei bhaut kuch present rehta hai ki video ka type kya hai, length kya hai,...etc.
    ...
    // resource_type: 'image',  //video k liye video kr dena 
    // transformation: [
    // { width: 400, height: 225, crop: 'fill' },
    // { quality: 'auto' },
    // { start_offset: 'auto' }  
    // ],
    // format: 'jpg' // mp4 kr dena video k liye
    // });
    //thumbnail apne aap bhi bana kr cloudinary pr dekh lena.
...
const deleteVideo = async (req, res) => {
    try {
        ...
        const video = await SolutionVideo.findOneAndDelete({problemId:problemId});//DB se delete ho jayga lekin cloudinary se nhi hoga.
        ...
        if (!video) {
         ...
         // cloudinary se delete kr rahe hai.
